...
class AutoEncoder(nn.Module):
    """basenet for fer2013"""
    def __init__(self, encoded_dim=256, patch_size=16, space_step=0.5):
    ...
    def infer(self, x, trg):
        ps = self.patch_size
        pn = x.shape[2]//ps
        x_out = torch.empty(x.shape).cuda()
        trg_out = torch.empty(x.shape).cuda()
        ...
        for i in range(pn):
            for j in range(pn):
                ...
                patch_trg = trg[:, :, i*ps:(i+1)*ps, j*ps:(j+1)*ps]
                x_p = self.encoder.forward(patch)
                trg_p = self.encoder.forward(patch_trg)

                x_latent.append(x_p)
                trg_latent.append(trg_p)
                # trg_latent.append(torch.add(trg_p, torch.mul((torch.sub(x_p, trg_p)), self.space_step)))

        x_latent = torch.stack(x_latent)
        trg_latent = torch.stack(trg_latent)

        for i in range(pn):
            for j in range(pn):
                cur_trg_latent = trg_latent[i*pn+j].unsqueeze(0)
                cur_dists = ((cur_trg_latent - x_latent)**2).sum(axis=2)
                cur_dists = cur_dists.squeeze(1)
                closest_index = torch.argmin(cur_dists)
                trg_latent[i*pn+j] = torch.add(trg_latent[i*pn+j], torch.mul((torch.sub(x_latent[closest_index], trg_latent[i*pn+j])), self.space_step))

        for i in range(pn):
            for j in range(pn):
                x_dec = self.decoder.forward(x_latent[i*pn+j, :, :])
                trg_dec = self.decoder.forward(trg_latent[i*pn+j, :, :])
                x_out[:, :, i*ps:(i+1)*ps, j*ps:(j+1)*ps] = x_dec
                trg_out[:, :, i*ps:(i+1)*ps, j*ps:(j+1)*ps] = trg_dec

        return x_out, trg_out
...
class AutoEncodermodel(BaseModel):
    ...
    def __init__(self, configuration):
        ...
        self.space_step = configuration['space_step']

        #Initialize model defined above
        self.model = AutoEncoder(encoded_dim=configuration['encoded_dim'], patch_size=self.patch_size, space_step=self.space_step)
        self.model.cuda()
    ...
    def forward(self):
        ...
        if (self.model.training):
            self.output, self.output_trg = self.model.forward(x, trg)
        else:
            self.output, self.output_trg = self.model.infer(x, trg)
        return self.output, self.output_trg

    #Computes the loss with the specified name (in this case 'total')
    def compute_loss(self):
        self.loss_bin = self.bin_loss(self.output, self.input)# + self.bin_loss(self.output_trg, self.target)
        self.loss_grey = self.greyscale_loss(self.output, self.input)# + self.greyscale_loss(self.output_trg, self.target)
        self.loss_total = self.loss_bin + self.loss_grey
